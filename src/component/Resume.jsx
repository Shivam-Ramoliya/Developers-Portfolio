...
const Resume = ({ resumePdfUrl, experience, education }) => {
      ...
      >
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-start">
          <Avatar
            ...
            fallbackText={isEducation ? item.institution : item.company}
          />
  ...
  const TimelineItem = ({ item, isEducation, index }) => (
    <div className="relative pl-8 md:pl-32 pb-10 group" key={index}>
      <div className="absolute left-4 md:left-16 top-0 h-full w-0.5 bg-linear-to-b from-sky-500/70 to-emerald-500/70"></div>
      <div
        className={`absolute left-2 md:left-14 top-3 w-4 h-4 ${isEducation ? "bg-sky-500" : "bg-emerald-500"
          } rounded-full border-4 border-white dark:border-slate-800 group-hover:scale-150 transition-transform duration-300`}
      ></div>
      <div
        className={`mb-2 md:mb-0 md:absolute md:left-0 md:top-8 text-xs md:text-sm font-semibold ${isEducation
  ...
  return (
    <section id="resume" className="py-12 md:py-20">
      <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center gap-3 sm:gap-4 mb-8 md:mb-12">
          <button
            onClick={() => setActiveTab("experience")}
            className={`px-6 py-2 sm:px-8 sm:py-3 rounded-full font-semibold text-sm sm:text-base transition-all duration-300 ${activeTab === "experience"
          ...
          <button
            ...
            className={`px-6 py-2 sm:px-8 sm:py-3 rounded-full font-semibold text-sm sm:text-base transition-all duration-300 ${activeTab === "education"
                ? "bg-linear-to-r from-sky-500 to-emerald-500 text-white shadow-lg"
                : "bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-slate-600"
              }`}
          >
